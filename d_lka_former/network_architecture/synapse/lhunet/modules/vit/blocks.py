...
class TransformerBlock_LKA_Spatial(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.LayerNorm(hidden_size)
        self.gamma = nn.Parameter(1e-6 * torch.ones(hidden_size), requires_grad=True)
        self.epa_block = SpatialAttention_LKA(input_size=input_size, hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads)
        self.conv51 = UnetResBlock(3, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
        self.conv8 = nn.Sequential(nn.Dropout3d(0.1, False), nn.Conv3d(hidden_size, hidden_size, 1))
...
class TransformerBlock_LKA_Channel(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.LayerNorm(hidden_size)
        self.gamma = nn.Parameter(1e-6 * torch.ones(hidden_size), requires_grad=True)
        self.epa_block = ChannelAttention_LKA(hidden_size=hidden_size, num_heads=num_heads)
        self.conv51 = UnetResBlock(3, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
        self.conv8 = nn.Sequential(nn.Dropout3d(0.1, False), nn.Conv3d(hidden_size, hidden_size, 1))
...
class TransformerBlock_LKA_Channel_norm(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.LayerNorm(hidden_size)
        self.gamma = nn.Parameter(1e-6 * torch.ones(hidden_size), requires_grad=True)
        self.epa_block = ChannelAttention_LKA_norm(hidden_size=hidden_size, num_heads=num_heads)
        self.conv51 = UnetResBlock(3, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
        self.conv8 = nn.Sequential(nn.Dropout3d(0.1, False), nn.Conv3d(hidden_size, hidden_size, 1))
...
class TransformerBlock_Deform_LKA_Channel(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.LayerNorm(hidden_size)
        self.gamma = nn.Parameter(1e-6 * torch.ones(hidden_size), requires_grad=True)
        self.epa_block = ChannelAttention_Deform_LKA(hidden_size=hidden_size, num_heads=num_heads)
        self.conv51 = UnetResBlock(3, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
        self.conv8 = nn.Sequential(nn.Dropout3d(0.1, False), nn.Conv3d(hidden_size, hidden_size, 1))
...
class TransformerBlock_Deform_LKA_Channel_sequential(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.LayerNorm(hidden_size)
        self.gamma = nn.Parameter(1e-6 * torch.ones(hidden_size), requires_grad=True)
        self.epa_block = ChannelAttention_Deform_LKA_sequential(hidden_size=hidden_size, num_heads=num_heads)
        self.conv51 = UnetResBlock(3, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
        self.conv8 = nn.Sequential(nn.Dropout3d(0.1, False), nn.Conv3d(hidden_size, hidden_size, 1))
...
class TransformerBlock_Deform_LKA_Spatial_sequential(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.LayerNorm(hidden_size)
        self.gamma = nn.Parameter(1e-6 * torch.ones(hidden_size), requires_grad=True)
        self.epa_block = SpatialAttention_Deform_LKA_sequential(input_size=input_size, hidden_size=hidden_size, proj_size=proj_size, num_heads=num_heads)   
        self.conv51 = UnetResBlock(3, hidden_size, hidden_size, kernel_size=3, stride=1, norm_name="batch")
        self.conv8 = nn.Sequential(nn.Dropout3d(0.1, False), nn.Conv3d(hidden_size, hidden_size, 1))
