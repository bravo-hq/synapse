...
def convert_id_to_task_name(task_id: int):
    ...
    if len(unique_candidates) == 0:
        raise RuntimeError(
            ...
            % (
                task_id,
                os.environ.get("d_lka_former_preprocessed")
                if os.environ.get("d_lka_former_preprocessed") is not None
                else "None",
                os.environ.get("RESULTS_FOLDER")
                if os.environ.get("RESULTS_FOLDER") is not None
                else "None",
                os.environ.get("d_lka_former_raw_data_base")
                if os.environ.get("d_lka_former_raw_data_base") is not None
                else "None",
            )
        )
